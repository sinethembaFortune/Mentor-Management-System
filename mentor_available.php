<section class="py-5">
    <div class="container px-4 px-lg-5 mt-5 card rounded-0 card-outline card-purple shadow">
        <div class="row">
            <div class="col-md-12">
            <center>
			 <?php $cabs = $conn->query("SELECT * from mentor where availability=0");?>
            <h1 class="display-4 fw-bolder">Available Mentors <span class="badge badge-success px-3 rounded-pill">Available</span></h1>
            <hr>
            </center>
                ...
                <div class="row gx-4 gx-lg-5 row-cols-1 row-cols-sm-1 row-cols-md-2 row-cols-xl-3" id="cab_list">
                    <?php 
                    $cabs = $conn->query("SELECT * from mentor where availability=1");
                    while($row= $cabs->fetch_assoc()):
                    ?>
                    <a class="col item text-decoration-none text-dark book_cab" href="javascript:void(0)" data-id="<?php echo $row['id'] ?>" data-bodyno="<?php echo $row['offer'] ?>">
                        <div class="callout callout-primary border-success rounded-0">
						 <dl>
							...
							<?php endif; ?> <?php echo $row['firstname'] ?>, <?php echo $row['lastname'] ?></dt>
                                <dd class="truncate-4 text-muted 2h-1">
                                    <small>Email: <?php echo $row['email'] ?></small><br>
                                    <small>Phone No. :<?php echo $row['mobile'] ?></small><br>
									<small>Stream :<?php echo $row['stream'] ?></small><br>
									<small>Offer :<?php echo $row['offer'] ?></small>
