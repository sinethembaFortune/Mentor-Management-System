...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		$chk = $this->conn->query("SELECT * FROM `admin` where username ='{$username}' ".($id>0? " and id != '{$id}' " : ""))->num_rows;
		if($chk > 0){
			return 3;
			exit;
		}
	...
	public function save_client(){
		...
		if(isset($_POST['oldpassword'])){
			if($this->settings->userdata('id') > 0 && $this->settings->userdata('login_type') == 2){
				$get = $this->conn->query("SELECT * FROM `mentee` where id = '{$this->settings->userdata('id')}'");
				$res = $get->fetch_array();
		...
		$check = $this->conn->query("SELECT * FROM `mentee` where email = '{$email}' and delete_flag ='0' ".(is_numeric($id) && $id > 0 ? " and id != '{$id}'" : "")." ")->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' Email already exists in the database.';
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `mentee` set $data";
			}else{
				$sql = "UPDATE `mentee` set $data where id = '{$id}'";
			}
			...
			if($save){
				...
				if(empty($id)){
					...
					$resp['msg'] = " Mentee's Account Details has been updated successfully.";
				}
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(isset($uploaded_img)){
						$this->conn->query("UPDATE mentee set `image_path` = CONCAT('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$uid}' ");
				...
				if(empty($id)){
					...
					$resp['msg'] = " Mentee's Account Details has failed to update.";
				}
			}
		}
	...
	function delete_client(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `mentee` set delete_flag = 1 where id='{$id}'");
		if($del){
			$resp['status'] = 'success';
