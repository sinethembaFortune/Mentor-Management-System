...
if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') == 2){
    $qry = $conn->query("SELECT * FROM `mentee` where id = '{$_settings->userdata('id')}'");
    if($qry->num_rows >0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k)){
    ...
    <div class="container">
        <div class="card card-outline card-purple shadow rounded-0">
			...
			<div class="form-group">
                ...
                <textarea name="studentno" id="studentno"class="form-control rounded-0" required><?php echo isset($studentno) ? $studentno : ''; ?></textarea>
			</div>
            <div class="form-group">
...
<script>
    ...
    $(function(){
        ...
        $('#register-frm').submit(function(e){
            ...
            start_loader();
            $.ajax({
                url:_base_url_+"classes/Users.php?f=save_mentee",
